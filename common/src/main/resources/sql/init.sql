...
CREATE TABLE `t_tenant_db_mapping` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT,
  `tenant_code` varchar(100) NOT NULL COMMENT '租户标识',
  `db_code` varchar(100) NOT NULL COMMENT '数据源标识',
  `type` varchar(50) NOT NULL COMMENT '数据源类型',
  `module` varchar(50) NOT NULL DEFAULT '' COMMENT '数据源所属子模块',
  `create_time` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  `update_time` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3),
...
) ENGINE=InnoDB AUTO_INCREMENT=0 DEFAULT CHARSET=utf8mb4 COMMENT='租户和数据源映射表'
